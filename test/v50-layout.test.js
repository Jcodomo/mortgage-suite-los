...
const checks=[
  ...
  ['The File group opens Setup by default, including after a late workspace mount',/function wireFileToSetup\(\)[\s\S]*?data-group="file"[\s\S]*?V50\.go\('SETUP'\)[\s\S]*?\[420,1300,2600\]\.forEach\(function\(ms\)\{ setTimeout\(wireFileToSetup,ms\); \}\)/.test(dist)],
