...
const checks=[
  ...
  ['Loan Suite restores the requested workspace on refresh and keeps a compact browser resume marker',/var RESUME_KEY='los\.v50\.resume', WORKSPACE_COOKIE='los\.v50\.workspace'[\s\S]*?function persistWorkspace\(next\)[\s\S]*?document\.cookie=[\s\S]*?function pinSuiteEntry\(\)[\s\S]*?preferredWorkspace\(\)/.test(dist)],
  ...
  ['The File group opens Setup by default',/function wireFileToSetup\(\)[\s\S]*?data-group="file"[\s\S]*?V50\.go\('SETUP'\)/.test(dist)],
