...
/* Mortgage Rates and Property became top-level tabs, and Taxes & Escrow
   folded into the Escrow tab — all three are handled by patch-v8.js. */
V4.advSub = 'rules';
...
function paintAdv(){
  ...
  var st = suite();
  var onAdvanced = st && st.snapshot.mode === 'advanced' && !(window.V8 && V8.active);
...
function syncMode(){
  if (window.V8 && V8.active) return;   /* a v8 pseudo-tab owns the stage */
