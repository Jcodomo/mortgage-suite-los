...
var THEME_LABEL = { light:'Light', dark:'Dark (black)', navy:'Navy blue' };
...
function paintThemeButton(){
  /* v9 extends the cycle to four themes and owns the button once loaded.
     Without this hand-off the two paint loops fight: on the OLED skin
     this table has no entry, so every 650ms the icon snapped back to the
     sun and the tooltip read "undefined — click to cycle". */
  if (window.V9 && V9.paintTheme){ V9.paintTheme(); return; }
  var btn = $('v5ThemeBtn'); if (!btn) return;
  var cur = (window.LOS && LOS.skin) ? LOS.skin() : 'light';
  btn.innerHTML = THEME_ICONS[cur] || THEME_ICONS.light;
  btn.title = (THEME_LABEL[cur] || cur) + ' — click to cycle';
}
...
function liveUpgrade(e){
  ...
  /* v10 converts number inputs to text and strips "$, " in its own
     capture listener — but that listener registers AFTER this one, so
     without this line the handler would run once against "$550,000",
     N() would read it as 0, and the figure would lurch until the next
     keystroke. Strip here first; v10's pass then finds nothing to do. */
