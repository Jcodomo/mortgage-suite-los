...
/* Keep the two workspace selectors reachable and keep their URL route in
   sync. Some older builds leave the original lexical controller unavailable
   to later patch layers; the DOM-backed bridge preserves the same switch. */
function ensureWorkspaceSwitch(){
  var calc=$('calc-root'), suite=$('suite-root'), calcBtn=$('mode-calc'), suiteBtn=$('mode-suite');
  if (!calc || !suite || !calcBtn || !suiteBtn) return false;
  var shell=window.SHELL;
  if (!shell || typeof shell.go!=='function'){
    shell={};
    shell.go=function(next){
      next=next==='suite'?'suite':'calc';
      calc.classList.toggle('on',next==='calc');
      suite.classList.toggle('on',next==='suite');
      calcBtn.classList.toggle('on',next==='calc');
      suiteBtn.classList.toggle('on',next==='suite');
      document.body.dataset.shellMode=next;
      if (next==='suite'){
        var s=store();
        if (s && s.snapshot && typeof s.setMode==='function') try { s.setMode(s.snapshot.mode); } catch(e){}
      }
      try {
        var url=new URL(location.href);
        url.searchParams.set('app',next==='suite'?'suite':'income');
        history.replaceState(history.state,'',url.href);
      } catch(e){}
      try { window.scrollTo({top:0,behavior:'smooth'}); } catch(e){}
    };
    shell.toSuite=function(){ shell.go('suite'); };
    shell.toCalc=function(){ shell.go('calc'); };
    shell.say=function(){};
    Object.defineProperty(shell,'mode',{get:function(){ return suite.classList.contains('on')?'suite':'calc'; }});
    shell.__v46Bridge=true;
    window.SHELL=shell;
  } else if (!shell.__v46RouteSync){
    var originalGo=shell.go.bind(shell);
    shell.go=function(next){
      var result=originalGo(next);
      try {
        var url=new URL(location.href);
        url.searchParams.set('app',next==='suite'?'suite':'income');
        history.replaceState(history.state,'',url.href);
    ...
    shell.__v46RouteSync=true;
  }
  if (!calcBtn.__v46Switch){ calcBtn.__v46Switch=true; calcBtn.onclick=function(){ window.SHELL.go('calc'); }; }
  if (!suiteBtn.__v46Switch){ suiteBtn.__v46Switch=true; suiteBtn.onclick=function(){ window.SHELL.go('suite'); }; }
  return true;
}
...
function tick(){ try { ensureWorkspaceSwitch(); } catch(e){} try { metricCards(); } catch(e){} try { aiPanel(); } catch(e){} }
