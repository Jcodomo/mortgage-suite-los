...
PROP.render = function(){
    ...
    + '</div><div class="card-body">'
      + '<div class="grid g4">'
        ...
        + (isTBD
            ...
            : '<div class="field" style="grid-column:span 2"><label>Property address</label><input class="cell-input" value="' + esc(s.address) + '" onchange="LOANSUITE.PROP.set(\'address\',this.value)" placeholder="Street, city, state, ZIP"></div>')
  ...
  /* The tax proration / aggregate panel used to be rendered here as well
     as on its own Advanced subtab, which put two elements with id
     "taxBody" in the document — TAXPRO.render() only ever found the
     first, so one of the two was permanently dead. It now lives in one
     place, under the Escrow tab's aggregate section (patch-v8.js). */
};
function provBlock(){
