...
function injectReportJsonButtons(){
  ...
  /* The bar is .rpt-modal-bar. The first draft guessed .rpt-actions /
     .rpt-foot / .modal-foot, none of which exist, so these buttons were
     silently never added — the exact failure mode a guessed selector
     produces. */
  var bar = modal.querySelector('.rpt-modal-bar');
  ...
  /* before the Close button, after the export buttons — export first,
     file round-trip second, leave last */
  var closeBtn = Array.prototype.filter.call(bar.querySelectorAll('button'),
    function(b){ return /close/i.test(b.textContent); })[0];
  save.classList.add('btn-sm'); load.classList.add('btn-sm');
  if (closeBtn){ bar.insertBefore(save, closeBtn); bar.insertBefore(load, closeBtn); }
  else { bar.appendChild(save); bar.appendChild(load); }
  return true;
}
...
/* v5's LOCKEXT.render is wrapped so the two date inputs are rewritten
   the moment the card renders — leaving it to the 500ms poll left a
   window where they were pickers again after every commit, the same
   defect the tax panel had. */
function wrapLockRender(){
  var LX = window.V5 && V5.LOCKEXT;
  if (!LX || typeof LX.render !== 'function' || LX.render.__v7) return false;
  var inner = LX.render;
...
V7.paintCapacity = function(){
  ...
  host.style.display = '';
  /* Two guards. While the cushion box holds the caret, no rebuild — a
     rebuild would eat the caret on every keystroke. Otherwise rebuild
     only when a figure actually moved: this runs on a 500ms poll, and
     rewriting identical innerHTML twice a second is pure waste plus a
     flicker of any hover state. */
  var live = document.activeElement;
  if (host.contains(live) && live.tagName === 'INPUT') return;
  var sig = JSON.stringify(c);
  if (host.__v7sig === sig) return;
  host.__v7sig = sig;
...
setInterval(function(){
  ...
  try { wrapLockRender(); upgradeLockFields(); } catch(e){}
  try { V7.paintCapacity(); } catch(e){}
}, 500);
})();
